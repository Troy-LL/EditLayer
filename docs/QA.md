...
`npm test` runs `shared/overlay/overlay.test.js` plus `client/src/*.test.js` (clone, lock inherit, canvas lock inherit, paste-into-frame tree, multi-dup, paste parent, atomic offsets). Overlay goldens:
